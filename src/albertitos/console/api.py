...
Sin dependencias nuevas (stdlib): siete GET, JSON en snake_case, CORS abierto en localhost. No escribe:
...
def despachar(
    ...
    if method not in ("GET", "HEAD"):
        return 405, {"error": "sólo GET (la consola es de sólo lectura)"}
...
def hacer_handler(ruta: Path) -> type[BaseHTTPRequestHandler]:
    class Handler(BaseHTTPRequestHandler):
        ...
        do_POST = do_PUT = do_PATCH = do_DELETE = _no_escritura  # noqa: N815
...
def servir(ruta: Path | None = None, puerto: int = PUERTO_DEFECTO) -> None:
    ...
    print(
        f"Albertitos consola API v{lecturas.API_VERSION} · sólo lectura · "
        f"http://127.0.0.1:{puerto}  (BD {ruta})"
    )
    print("Rutas: /salud /panel /ficheros /ficheros/:id /traza /etapas /eventos")
    try:
        httpd.serve_forever()
