...
| `/inbox` | `{ estado: idle\|ingiriendo\|extrayendo\|decidiendo\|listo\|error, file_ids, ficheros[{file_id, estado}], log[], error, lote: 99, disponible, bd }` | memoria del puente + `lecturas.estados` |
...
en curso. **Sólo con `--bandeja`** (sirve `dist/bandeja.db`, copia de la real si no existe; bórrala para empezar de
cero): contra `dist/albertitos.db` responde **409**, porque un fichero de lote 99 allí deja la auditoría en ROJO
(`comprobar_fantasmas`, package se niega) y el siguiente `run` lo mete en `marcar_duplicados`. Una copia exacta de un
PDF de la Caja entra en `identidades` y hereda su decisión sin LLM. Añadir la ruta no sube `API_VERSION`.
...
- Pendiente de Miguel (Makefile): `console-api: uv run python -m albertitos.console.api` · `console-web: cd console-web && pnpm dev`.
