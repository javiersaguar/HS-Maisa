...
from albertitos.extract import instrucciones, pdf, plantillas, validadores
...
def _segunda_lectura(
    ...
    Después, lo que digan las dos lecturas en `texto_sospechoso` (`_evidencia_de_lecturas`)."""
    try:
    ...
    return _evidencia_de_lecturas(
        llm.conn, hechos, uso, [("principal", hechos), (f"sup{DPI_VISION_2}", otra)]
    )
...
def _evidencia_de_lecturas(
    ...
    """Lo que las lecturas de una escaneada dicen además de sus campos: su `texto_sospechoso`.

    - Si un fragmento nombra a otro proveedor del maestro → DOCUMENTO_SUPERPUESTO, con la evidencia en el
      evento y NO en `texto_sospechoso`: ese campo es para instrucciones y R6 lo cita como "el documento
      dice". Si la lectura principal ya lo había tomado por instrucción, se deja como estaba: ante la
      duda, escalar.
    - Si una lectura que no es la principal trae una instrucción que casa con las regex de
      `instrucciones.py`, y la principal no traía ninguna → TEXTO_INSTRUCCION con ese fragmento. Lo que
      no casa (un sello, una nota) no se adopta: la relectura en frío del 18/09 tomó por instrucción un
      "RECIBIDO CONTABILIDAD" y escaló una factura limpia (docs/benchmark.md, «Determinismo»).
    Sin maestro, o si no se sabe de qué proveedor es la factura, no se marca nada: no hay contra qué
    comparar.
    """
    uso = dict(uso)
    maestro = _maestro(conn)
    propio = _proveedor_de_la_factura(hechos, maestro) if maestro is not None else None
    superpuestas: set[str] = set()
    for nombre, lectura in lecturas:
        fragmento = lectura.texto_sospechoso
        if not fragmento or maestro is None or propio is None:
            continue
        otro = _proveedor_nombrado(fragmento, maestro, excluir=propio.id)
        if otro is None:
            continue
        ...
        if "superpuesto" not in uso:
            ...
            uso["superpuesto"] = {
                ...
                "fragmento": fragmento[:160],
            }
        if Aviso.DOCUMENTO_SUPERPUESTO not in hechos.avisos:
            hechos.avisos.append(Aviso.DOCUMENTO_SUPERPUESTO)
    if hechos.texto_sospechoso is None:
        for nombre, lectura in lecturas[1:]:
            fragmento = lectura.texto_sospechoso
            if (
                fragmento
                and nombre not in superpuestas
                and instrucciones.detectar_instruccion(fragmento)
            ):
                hechos.texto_sospechoso = fragmento
                if Aviso.TEXTO_INSTRUCCION not in hechos.avisos:
                    hechos.avisos.append(Aviso.TEXTO_INSTRUCCION)
                uso["instruccion_de"] = nombre
                break
    return uso
