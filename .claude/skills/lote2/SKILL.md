...
El verificador comprueba NFC, colisiones por mayúsculas/tildes y con lote 1, apertura de PDF y columnas/filas del CSV; `--esperados 40` exige el recuento. **PDF idénticos (misma SHA-256, otro nombre) — P0-1, `docs/agentes/P0-1-IDENTICOS.md`:** sin el parche, el verificador los **para** (ROJO «PDF idéntico por SHA-256»): ingest reasignaría el original o colapsaría los dos nombres, y las dos salidas son NO APTO. Qué hacer: que Miguel aplique `dist/ensayo/h1/identicos.patch` (`git apply … && make check`, 10 min) y repetir la verificación. Con el parche aplicado, sale **AVISO «copia exacta…»**: se sigue normalmente; cada nombre tendrá su línea en su lote y todas las copias (también el original del lote 1) saldrán ESCALAR por `marcar_duplicados`, que corre dentro de `run` y de `reprocess`. Comprobar después con `albertitos trace <copia>`: `mismo_pdf_que` nombra a la otra. Nunca modificar un PDF ni un nombre oficial para esquivar el control. Bloquea `.PDF` y subcarpetas que ingest omitiría. Los PDF fuera de `facturas/` se muestran como adjuntos. Sin esa carpeta, se consideran facturas salvo nombres de norma/regla/manual. Revisar la clasificación si la estructura es otra. Si la regla llega por el canal, conservarla literalmente con su procedencia para Mónica.
