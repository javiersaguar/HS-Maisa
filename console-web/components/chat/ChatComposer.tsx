  ...
  return (
    <div className="border-t border-[#e1e5df] bg-white px-4 py-3">
      <div className="mb-2 flex flex-wrap gap-1" aria-label="Atajos de pregunta">
        {ATAJOS.map(tipo => <button key={tipo} type="button" aria-pressed={atajo === tipo} onClick={() => { setAtajo(atajo === tipo ? null : tipo); setValor('') }} className=" border border-[#e1e5df] px-2 text-[11px] text-[#164f45] aria-pressed:bg-[#eff8f3]">{tipo}</button>)}
      </div>
      {atajo ? <div className="mb-2 bg-[#f7f8f5] p-2 text-xs">
        <label htmlFor={id + '-atajo'}>{atajo === 'Semana' ? 'Semana ISO (vacío: esta semana)' : 'Identificador de ' + atajo.toLowerCase()}</label>
        <div className="mt-1 flex gap-2">
          <input id={id + '-atajo'} value={valor} onChange={e => setValor(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && prevista) { e.preventDefault(); cambiar(prevista); setAtajo(null); campo.current?.focus() } }} className="min-w-0 flex-1 border border-[#e1e5df] bg-white px-2 py-1" />
      ...
      <form onSubmit={e => { e.preventDefault(); enviar(entrada) }} className="flex items-end gap-2">
        ...
        <textarea id={id} ref={campo} value={entrada} onChange={e => cambiar(e.target.value)} onKeyDown={e => { if (e.key === 'Enter' && !e.shiftKey && !e.nativeEvent.isComposing) { e.preventDefault(); enviar(entrada) } }} rows={2} placeholder="Pregunta lo que necesitas saber…" className="min-h-14 max-h-28 min-w-0 flex-1 resize-none border border-[#e1e5df] px-3 py-2 text-[13px] leading-6 text-[#17211e]" />
        <button type="submit" disabled={enviando || !entrada.trim() || entrada.trim().length > MAX_MENSAJE} aria-label="Enviar la pregunta" className="chat-press flex size-11 shrink-0 items-center justify-center bg-[#164f45] text-white disabled:opacity-40"><SendHorizontal className="size-4" /></button>
      </form>
