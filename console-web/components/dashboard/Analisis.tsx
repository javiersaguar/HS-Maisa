...
function Regla({ motivo }: { motivo: Motivo }) {
  ...
  return (
    <li className="flex gap-3 border-t border-[#edf0ec] py-3 first:border-t-0 first:pt-0">
      <span
        aria-hidden
        className={`mt-[3px] h-4 w-[3px] shrink-0 rounded-sm ${motivo.ok ? 'bg-[#bcd8cb]' : 'bg-[#c79a15]'}`}
      />
      <div className="min-w-0">
        <p className="text-[12px] font-semibold uppercase tracking-[0.1em] text-[#8b9790]">
          {tituloRegla(motivo.regla_id)}
          {!motivo.ok && <span className="ml-2 normal-case tracking-normal text-[#856400]">no se cumple</span>}
        </p>
        <p className="mt-1 text-[14px] leading-relaxed text-[#2c3632]">{frase(motivo.detalle)}</p>
        {evidencia.length > 0 && (
          <ul className="mt-1.5 space-y-0.5">
            {evidencia.map((linea) => (
              <li key={linea} className="text-[13px] leading-relaxed text-[#68736d]">
                {linea}
              </li>
            ))}
          </ul>
...
function Dato({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="min-w-0">
      <p className="text-[11px] uppercase tracking-[0.1em] text-[#9aa39e]">{label}</p>
      <p className="mt-1 truncate text-[14px] font-medium text-[#17211e] tabular-nums">{children ?? '—'}</p>
    </div>
  )
}
...
function Contenido({ fichero }: { fichero: Fichero }) {
  ...
  return (
    <div className="flex min-h-0 flex-1 flex-col">
      <div className="flex items-start justify-between gap-4 border-b border-[#e5e8e3] px-6 py-5">
        <div className="min-w-0">
          <h2 className="truncate text-[17px] font-semibold tracking-[-0.01em]">{fichero.file_id}</h2>
        ...
        <Link
          ...
          className="flex shrink-0 items-center gap-1 text-[13px] font-semibold text-[#176d59] hover:underline"
        >
          Ficha completa
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-6 py-5">
        ...
        {decision ? (
          <>
            <p className="mt-5 text-[15px] leading-relaxed text-[#2c3632]">{resumenReglas(decision)}</p>
            ...
            {hechos?.texto_sospechoso && (
              <figure className="mt-4 border-l-2 border-[#c79a15] bg-[#fffaec] px-4 py-3">
                <figcaption className="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#856400]">
                  El documento intenta decidir por su cuenta
                </figcaption>
                <blockquote className="mt-1.5 text-[13px] leading-relaxed text-[#6b5400]">
                  «{hechos.texto_sospechoso}»
                </blockquote>
                <p className="mt-1.5 text-[12px] text-[#8a7a4a]">
            ...
            {cumplen.length > 0 && (
              <details className="mt-4 border-t border-[#edf0ec] pt-3">
                <summary className="cursor-pointer text-[13px] font-medium text-[#68736d] hover:text-[#176d59]">
          ...
          <p className="mt-5 text-[15px] leading-relaxed text-[#2c3632]">
        ...
        {hechos && (
          <section className="mt-6 border-t border-[#edf0ec] pt-5">
            <h3 className="text-[12px] font-semibold uppercase tracking-[0.12em] text-[#8b9790]">
  ...
  if (!fileId) {
    return (
      <div className="flex flex-1 flex-col items-center justify-center px-8 py-16 text-center">
        <FileSearch className="size-8 text-[#c3cdc7]" />
        <p className="mt-4 max-w-[34ch] text-[15px] font-medium text-[#59635e]">
          Elige una factura de la bandeja y aquí verás qué decide la norma y con qué argumentos.
        </p>
        <p className="mt-2 max-w-[42ch] text-[13px] leading-relaxed text-[#9aa39e]">
