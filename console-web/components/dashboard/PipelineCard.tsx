...
import { ETAPA_DESCRIPCIONES, ETAPA_LABELS, formatMs, formatNumber } from '@/lib/format'
import { Card } from '@/components/ui/Card'
import { ProgressRing } from '@/components/ui/ProgressRing'
import { StatusBadge } from '@/components/ui/StatusBadge'
import { EtapaIcon, saludEtapa } from '@/components/workers/EtapaIcon'

/** Columna izquierda del panel: las seis etapas del pipeline y cuántos ficheros ha completado cada una. */
export function PipelineCard({ etapas, ficheros, actions }: { etapas: EtapaResumen[]; ficheros: number; actions?: ReactNode }) {
  const alDia = etapas.filter((etapa) => saludEtapa(etapa, ficheros).tone === 'green').length
  return (
    <Card className="flex min-h-[720px] flex-col overflow-hidden">
      <div className="flex items-center justify-between border-b border-[#e5e8e3] px-5 py-4">
        <div>
          ...
          <p className="text-[14px] text-[#9aa39e]">Ficheros completados por etapa</p>
        </div>
        <div className="flex items-center gap-2">
          <StatusBadge tone={alDia === etapas.length ? 'green' : 'yellow'}>
          ...
          return (
            ...
            >
              <div className="flex min-w-0 items-center gap-4">
                <div
                  className={`flex size-8 shrink-0 items-center justify-center rounded-lg transition-colors duration-500 ${ok ? 'bg-[#effaf6] text-[#176d59]' : 'bg-[#fff9e6] text-[#a87000]'}`}
                >
                ...
                <div className="min-w-0">
                  ...
                  <p className="mt-1 text-[13px] text-[#8d9891]">
                    {formatNumber(etapa.ficherosOk)} ficheros · media {formatMs(etapa.latenciaMediaMs)}
                    {etapa.reintentos > 0 ? ` · ${etapa.reintentos} reintentos` : ''}
                  </p>
                </div>
              </div>
