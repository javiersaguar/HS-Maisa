...
export function InvoiceDocument({
  ...
  const box = (campo: CampoHecho) => {
    ...
    const base = highlight || active ? 'border-accent bg-raised' : 'border-line bg-surface'
    return `${base} ${active ? 'ring-2 ring-accent ring-offset-2 scale-[1.02]' : ''} transition duration-200`
  }
  ...
  return (
    <Card className="flex min-h-[690px] flex-col overflow-hidden border-line">
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-line bg-surface px-4 py-3">
        <div className="flex items-center gap-1 border border-line bg-surface p-1">
          <button
            ...
            disabled={zoom <= 60}
            aria-label="Alejar"
            className="size-7 min-h-0 text-sm transition hover:bg-raised disabled:opacity-30"
          >
            −
          </button>
          <button
            ...
            className="min-h-0 min-w-12 text-center text-[13px] font-semibold tabular-nums transition hover:bg-raised"
          >
            {zoom}%
          </button>
          <button
      ...
      <div className="flex-1 overflow-auto bg-raised p-5 sm:p-8">
        <div
          ref={pageRef}
          className="relative mx-auto flex min-h-[610px] max-w-[820px] flex-col overflow-hidden border border-line-soft bg-surface p-7 transition-transform duration-200 ease-out"
        ...
        >
          {!hechos && (
            <div aria-hidden="true" className="pointer-events-none absolute inset-0 z-10 bg-surface/60">
              ...
              <span className="absolute right-3 top-3 text-[12px] font-medium text-warn">
          ...
          <div className="flex items-start justify-between">
            ...
            <div className="flex flex-col items-end gap-2">
              <span className="text-[18px] font-bold tracking-[0.2em] text-muted">FACTURA</span>
              <span className="h-2 w-14 bg-raised" />
            </div>
          </div>
          ...
          <div className="mt-3 h-px bg-raised" />
            ...
            {hechos?.lineas.length
              ...
              : [1, 2, 3].map((line) => (
                  <div key={line} className="flex justify-between">
                    <span className="h-2 w-44 bg-raised" />
                    <span className="h-2 w-10 bg-raised" />
                  </div>
                ))}
          </div>

          {hechos?.texto_sospechoso && (
            <div className="mt-5 border border-dashed border-warn px-3 py-2">
