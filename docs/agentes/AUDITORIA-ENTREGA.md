...
| Decisión tomada con unos hechos que ya no son los de la BD | ROJO | Se reextrajo o se importó y no se volvió a decidir: lo entregado no sale de lo que hay. Incluye la **contingencia que ya tiene hechos** («contingencia (ADR-0009) y ya hay hechos») | `reprocess --impacted` |
| Decisiones de contingencia (ESCALAR sin hechos, ADR-0009) | ÁMBAR | Ficheros del lote 2 que se entregan como ESCALAR con `"regla":"contingencia.C1"` porque nadie pudo leerlos a tiempo (`scripts/contingencia.py`), con el último error y el motivo que se dio | Es una salida prevista: no bloquea, pero quien entrega tiene que verla. Si el proveedor vuelve antes de entregar: `extract --workers 4` + `reprocess --impacted` las sustituyen por la decisión de la norma |
