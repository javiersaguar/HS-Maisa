...
de 92 líneas, y ninguna cifra de la defensa tiene escrita su fuente y el comando que la reproduce (el 0,22 f/s
de visión llegó al plan antes de corregirlo; Miguel lo arregló en el PR #1, pero nada impide que vuelva a pasar). Todo eso es de `sources/` y de las cifras: es mío.
...
1. **Contingencia: Miguel ha dicho SÍ con cuatro condiciones** (09:10, recogidas abajo en el prompt de G1 como
   requisitos, no como sugerencias). **Falta el sí de Mónica** (pregunta C de `docs/PLAN-MONICA.md`). G1 la construye
   igual; ADR-0009 queda en *propuesto* hasta tener los dos síes, y nadie la aplica a la BD real antes.
2. Lo de siempre:
```bash
...
CONDICIONES DE MIGUEL (dueño de pipeline/ y del merge; aceptó la contingencia a las 09:10 con estas cuatro, y son requisitos: si algo del resto del prompt las contradice, mandan ellas):
  C1 · MANUAL Y ÚLTIMO RECURSO. Ni `run` ni `package` la aplican nunca por su cuenta: sólo `scripts/contingencia.py --aplicar --motivo "…"`, y después de haber reintentado de verdad (caos apagado, extract de los pendientes, modelo de respaldo). El script tiene que COMPROBAR lo que se pueda comprobar: se niega a aplicar con el caos encendido en esa BD y, por cada fichero, enseña los intentos de extract que tuvo (cuántos, cuándo, con qué modelo y error, y si alguno fue con el respaldo). (Pista: los fallos simulados llevan `caos:` en el `detalle` del evento; compruébalo en `extract/llm.py` antes de fiarte.) Si un fichero no tiene ningún intento con el caos apagado, se niega a aplicarla a ese fichero y dice el comando de extract que hay que lanzar antes.
  C2 · SÓLO ESCALAR, sólo ficheros SIN decisión vigente y NUNCA en el lote 1 (con `--lote 1` se niega siempre, aunque haya pendientes: eso sería otro problema y hay que verlo).
  C3 · REVERSIÓN PROBADA POR LOS DOS CAMINOS, cada uno con su test: `reprocess --impacted` y `run` entero. En los dos, cuando llegan los hechos la contingencia queda con vigente=0 y la nueva decisión es la de la norma.
  C4 · QUE SE VEA en tres sitios, cada uno con su test: `"regla":"contingencia.C1"` en la línea de outcomes que escribe `package` (package pone en `regla` la primera regla que falla, así que el Motivo de la contingencia tiene que ser el único con ok=False); un evento `decide` con `"contingencia": true` en el detalle; y ÁMBAR en la auditoría. Mientras `package` no ejecute la auditoría (punto G), la skill /lote2 dice que quien entrega pasa antes `make publicar` o `scripts/auditoria_entrega.py`.
...
B · tests/test_contingencia.py (fixture `conn` de conftest; sin red ni LLM): en seco no escribe; con --aplicar sólo toca a los que no tienen decisión; todo lo que escribe es ESCALAR; es idempotente; sin `--motivo` se niega; con `--lote 1` se niega; con el caos encendido se niega; un fichero sin ningún intento de extract con el caos apagado no se toca (C1); `package` pasa de negarse a APTO y la línea del fichero lleva `"regla":"contingencia.C1"`; el evento decide lleva `"contingencia": true` (C4); REVERSIÓN por `reprocess --impacted` y, en otro test, por `run` entero: la contingencia queda con vigente=0 y la vigente es la de la norma (C3).
...
D · Skill /lote2: un paso nuevo, "Si a las 07:30 del domingo queda algún PENDIENTE", con la secuencia en seco → --aplicar con motivo → auditoría → make publicar, y la regla de que antes se intenta de verdad (chaos apagado, reintentar el extract de los pendientes, el modelo de respaldo). La contingencia es lo último, no un atajo. Añade las cuatro condiciones de Miguel tal cual y, mientras `package` no audite, el paso obligatorio `make publicar` (en seco) o `scripts/auditoria_entrega.py` antes de entregar.
E · ADR-0009, "Si a la hora de entregar no hay hechos, ESCALAR explícito y reversible", en estado **propuesto** (Miguel ya dijo sí con condiciones; falta Mónica), con las cuatro condiciones en la sección Decisión: contexto (binario 540/540), alternativas (no entregar el lote 2; PAGAR por defecto; NO_PAGAR por defecto; ESCALAR con registro), decisión, consecuencias y evidencia (la de tu ensayo) + "Resumen para el plan (5 líneas)" como los demás. Añádelo a docs/adr/README.md.
...
CRITERIOS DE ACEPTACIÓN: las cuatro condiciones de Miguel, cada una con su test; sin --aplicar no escribe nunca; sólo escribe ESCALAR; se revierte sola con los hechos por los dos caminos (tests y ensayo); la auditoría la enseña en ámbar; la skill /lote2 la tiene como último recurso; ADR-0009 propuesto; `make check` y `make agentes-check` verdes; la BD real y dist/entrega, con el mismo sha256 antes y después (apúntalos).
...
D · scripts/cifras_check.py (+ tests/test_cifras.py): lee docs/CIFRAS.md, donde cada cifra obsoleta tiene su forma escrita (p. ej. "0,22 ficheros/s"), y busca esas formas en docs/plan/albertitos_plan.md, docs/guion-defensa.md, docs/agentes/KIT-DEFENSA.md y docs/benchmark.md. Imprime fichero:línea, la cifra vieja y la vigente con su fuente. Sale 1 si encuentra alguna. Sólo lee: NO corrige los ficheros de otros. Ejecútalo contra el repo y pon la salida en tu parte. Para probarlo: el "0,22 ficheros/s" del plan ya lo corrigió Miguel en el PR #1, pero sigue en el historial y en docs/benchmark.md podría quedar alguna forma. El test tiene que encontrarlo en un fichero de prueba con esa línea. Si en el repo no sale nada, dilo: es una buena noticia. Lo que salga va con PIDO A Alfonso, con cada línea y el texto de sustitución.
...
CRITERIOS DE ACEPTACIÓN: sin ERP, `erp pull` falla con un mensaje de una línea en cuanto Miguel aplique el parche (y el test de sources lo fija ya); `resumen_erp` sobre la BD real da los números reales; CIFRAS.md tiene cada cifra con su fuente y su comando; cifras_check.py encuentra una cifra obsoleta en su test y dice lo que haya en el repo real; los PIDO con su parche en la bitácora; `make check` y `make agentes-check` verdes; la BD real y dist/entrega con el mismo sha256 antes y después.
...
sobre la contingencia. Con los dos síes, ADR-0009 pasa a aceptado y Javier cambia la regla 6 de CLAUDE.md a: «Si el LLM falla, el fichero queda PENDIENTE; a la hora de entregar, la contingencia ESCALAR de ADR-0009». Si ya se aplicó lo de `scan_025`: `make kit-demo` y el
