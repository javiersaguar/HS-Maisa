...
## Cambios de la web (18/09 ~23:00) que afectan al plan
- Material inicial = el repo de participantes compartido a las 19:00 (ya no hay zip de la Caja a las 21:00): `data/caja` es el oficial. Sólo el lote 2 llega como zip con hash.
- El sábado 18:00 trae "40 facturas, ERP actualizado" y "una regla nueva"; la web ya no dice "norma v4" ni promete un fichero: la regla puede llegar por el canal, en el zip o en el Excel (skill `lote2` y `hitos.md` actualizados).
- Cierre oficial **domingo 11:00** (commit registrado a esa hora); mantenemos 10:30 como cierre interno. Defensa: hora por confirmar.
- Bloque 4 de la defensa: "demostrad un **timeout**, rate limit, respuesta inválida o caída del proveedor" → C1 añade el escenario de timeout al guion de caos (B2 cubrió los otros tres).
- Bonus: explícitamente distinto del lote 2 y sin partes del flujo principal; máximo total 110.
...
|---|---|---|
| **C1** | Contraste LLM de las 468 de plantilla (100 %), tercera lectura medida para los escaneados difíciles, timeout corto con reintento **y escenario de timeout demostrable** (la web del 18/09 23:00 lo pide explícitamente en el bloque 4), fechas imposibles blindadas | `src/albertitos/extract/*` · `src/albertitos/sources/chaos.py` · `tests/test_{llm,extract,plantillas}.py` · `scripts/bench_llm.py` · `data/fixtures/hechos_{caja,muestra}.jsonl` · `docs/agentes/CONTRASTE-TOTAL.md` |
...
1. Sólo editas: src/albertitos/extract/* · src/albertitos/sources/chaos.py · tests/test_llm.py · tests/test_extract.py · tests/test_plantillas.py · scripts/bench_llm.py · data/fixtures/hechos_caja.jsonl · data/fixtures/hechos_muestra.jsonl · docs/agentes/CONTRASTE-TOTAL.md. Cualquier otro fichero: NO lo toques; escribe `PIDO A C2:`/`PIDO A Miguel:`/`PIDO A Mónica:` en docs/agentes/BITACORA.md y sigue.
...
4. Timeout y cola larga: baja el timeout de lectura por petición a 60 s (configurable `ALBERTITOS_LLM_TIMEOUT_S`) manteniendo los reintentos; repite `scripts/bench_llm.py` (o el subconjunto que tarde < 5 min) con marca nueva y comprueba que la cola de 94 s se convierte en reintento y no en espera. Cifra antes/después a CONTRASTE-TOTAL.md. ADEMÁS, escenario de timeout demostrable (la web actualizada pide en el bloque 4 de la defensa "un timeout, rate limit, respuesta inválida o caída"): añade el modo `llm_timeout` a `sources/chaos.py` (MODOS) y hazlo respetar en `llm.py` (la llamada lanza `ErrorLLM("LLM-TIMEOUT")` tras esperar como si el proveedor no contestara, o fuerza `timeout` mínimo en httpx), con reintentos visibles en eventos (`intento`, `error_codigo=LLM-TIMEOUT`) y PENDIENTE al agotarlos; test offline; salida literal del guion a CONTRASTE-TOTAL.md. `PIDO A Miguel:` el flag `chaos --llm-timeout` en cli.py (mientras, `python -c 'from albertitos.sources import chaos; chaos.activar("llm_timeout")'`).
