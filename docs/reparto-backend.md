...
| M5 | **Sábado 18:00: la regla nueva** (la web del 18/09 23:00 ya no la llama "norma v4" ni promete fichero; el README upstream tampoco garantiza ya la actualización del ERP: preparad los dos casos). Copiar `norma_v3.py` → `norma_v4.py`, cambiar sólo lo que cambie, `REGISTRO["v4"]`, tests de lo nuevo, ADR con el diff v3→v4 y qué decisiones toca (Miguel lo usa para `reprocess --impacted --norma v4`) | `norma_v4.py`, `__init__.py`, `test_rules_v4.py`, `docs/adr/000N-norma-v4.md` | `decide --norma v4` corre; ADR lista las reglas cambiadas |
