# Qué queda del backend para terminar · sábado 19/09, 09:50

**Resumen.** El backend hace todo lo que la entrega exige y está ensayado. Con los datos de hoy, 500 de 500 facturas
tienen decisión (443 PAGAR · 48 ESCALAR · 9 NO_PAGAR) y la entrega de seguro está publicada (`fdc76e8`). El lote 2 se
...
Estado de las ramas: `javier/ingesta` va **24 commits por delante de `main`** y nadie tiene commits propios sin
mergear. Mónica, Alfonso y Alejandro **no han subido nada todavía hoy**.
...
Respuesta B (los 5 PAGAR reconciliados → ADR-0003), muestra etiquetada (**0 de 21**), frontera NO_PAGAR/ESCALAR
...
## 4 · Lo de Javier hoy
1. **En cuanto Mónica responda la A:** aplicar el arreglo de `scan_025` a la BD real → auditoría en verde →
   `make publicar ARGS=--publicar` → `make kit-demo` y el kit nuevo a Alfonso → aplicar el parche de la puerta de G1.
